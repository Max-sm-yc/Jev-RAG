...
# Retrieve a broad candidate set, then retain the best chunks after LLM scoring.
...
log("config", f"Chat model and reranker: {CHAT_MODEL}")
...
# ---------------------------------------------------------------------
# LLM re-ranking with Muse Spark
# ---------------------------------------------------------------------
def score_candidate_with_llm(question: str, candidate: Document) -> tuple[float, float]:
    """Score one candidate chunk using the configured Muse Spark chat model."""
    rerank_prompt = f"""
You are a passage relevance reranker.
...
""".strip()
    ...
    log("rerank", f"Calling {CHAT_MODEL} for {source_name(candidate)}")
    ...
    try:
        ...
        raise RuntimeError(
            f"{CHAT_MODEL} returned an invalid reranking response: {preview(content, 240)}"
        ) from exc
    ...
    log(
        "rerank",
        f"Received score={score:.3f}, confidence={confidence:.3f} "
        f"in {elapsed(started)}",
        color="green",
    )
...
def rerank(state: State) -> dict:
    """Present every retrieved candidate to Muse Spark and rank by relevance."""
    candidates = state.get("candidates", [])
    scored_candidates = []
    started = time.perf_counter()
    log("rerank", f"Scoring {len(candidates)} candidate(s) with {CHAT_MODEL}")
    ...
    for retrieval_rank, candidate in enumerate(candidates, start=1):
        ...
        score, confidence = score_candidate_with_llm(state["question"], candidate)
        reranked_candidate = Document(
            page_content=candidate.page_content,
            metadata={
                **candidate.metadata,
                "retrieval_rank": retrieval_rank,
                "reranker_relevance_score": score,
    ...
    scored_candidates.sort(
        key=lambda document: (
            document.metadata["reranker_relevance_score"],
            document.metadata["reranker_confidence"],
            -document.metadata["retrieval_rank"],
        ),
        reverse=True,
    )

    log("ranking", "Final Muse Spark ranking:", color="yellow")
    for rank, document in enumerate(scored_candidates, start=1):
        selected = "SELECTED" if rank <= RERANKED_DOCUMENTS else "dropped"
        log(
            "ranking",
            f"#{rank:02d} score={document.metadata['reranker_relevance_score']:.3f} "
            f"confidence={document.metadata['reranker_confidence']:.3f} "
...
def generate(state: State) -> dict:
    ...
    docs_content = "\n\n".join(
        (
            ...
            f"Relevance score: {document.metadata.get('reranker_relevance_score', 'not scored')}\n"
